...
module.exports = class extends Generator {
  prompting() {
    ...
    return this.prompt([
      ...
      },
      {
        ...
        choices: [
          ...
          'google',
          {
            name: 'twitter',
            disabled:
              'Soon - PRs are welcome (see: https://github.com/diegohaz/generator-rest/issues/8)',
          },
        ],
        when: function(props) {
      ...
      },
      {
        ...
        when: function(props) {
          return (
            props.generateAuthApi &&
            props.authMethods.indexOf('password') !== -1
          );
        },
      },
      {
        ...
        name: 'passwordReset',
        message:
          'Do you want to generate password reset API (it will need a SendGrid API Key)?',
        default: false,
        when: function(props) {
          return (
            props.generateAuthApi &&
      ...
      },
      {
        ...
        name: 'sendgridKey',
        message:
          "What's your SendGrid API Key (you can skip this and update the .env file later)?",
        default: 'key',
        when: function(props) {
      ...
      },
      {
        ...
        name: 'getList',
        message:
          'Do you want the retrieve methods from users (GET) to have the form { rows, count } ?',
        default: false,
        when: function(props) {
      ...
      that.props.passwordSignup =
        props.authMethods && props.authMethods.indexOf('password') !== -1;
      that.props.authServices =
        props.authMethods &&
        props.authMethods.filter(function(method) {
  ...
  writing() {
    ...
    copy(tPath('babel.config.js'), dPath('babel.config.js'));
    copy(tPath('prettier.config.js'), dPath('prettier.config.js'));
    ...
    copyTpl(
      tPath('services/response'),
      dPath(props.srcDir + '/services/response'),
      props
    );
    copyTpl(
      tPath('api/index.js'),
      dPath(props.srcDir + '/' + props.apiDir + '/index.js'),
      props
    );

    if (props.generateAuthApi) {
      copyTpl(
        tPath('services/passport'),
        dPath(props.srcDir + '/services/passport'),
        props
      );
      copyTpl(
        tPath('services/jwt'),
        dPath(props.srcDir + '/services/jwt'),
        props
      );
      copyTpl(
        tPath('api/user'),
        dPath(props.srcDir + '/' + props.apiDir + '/user'),
        props
      );
      ...
      if (props.authMethods.length) {
        copyTpl(
          tPath('api/auth'),
          dPath(props.srcDir + '/' + props.apiDir + '/auth'),
          props
        );
      }
      ...
      props.authServices.forEach(function(service) {
        copyTpl(
          tPath('services/' + service),
          dPath(props.srcDir + '/services/' + service),
          props
        );
      });
    }
    ...
    if (props.sendgridKey) {
      copyTpl(
        tPath('services/sendgrid'),
        dPath(props.srcDir + '/services/sendgrid'),
        props
      );
    }
  }
